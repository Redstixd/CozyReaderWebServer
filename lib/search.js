...
/**
 * 把搜索筛选参数解析成具体 adapter。
 * 前端书源标签可能传 adapter.id（"ranwen8"）或 source.name（"笔趣阁小说吧"），
 * 两者都要能命中：先按 id 查，再按 name 匹配。
 */
function resolveSource(adapters, source) {
  if (adapters.has(String(source))) return adapters.get(String(source));
  for (const a of adapters.values()) {
    if (a.name === source) return a;
  }
  return null;
}
...
async function aggregateSearch(cfg, adapters, params) {
  ...
  // 选源：指定 source 或全部启用源
  // source 可能是 adapter id（如 ranwen8）或书源名（如 笔趣阁），两者都要能解析
  let sources;
  if (params.source) {
    const adapter = resolveSource(adapters, params.source);
...
module.exports = { aggregateSearch, normalizeResult, isCoverCached, resolveSource };