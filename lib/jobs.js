 ...
 *   - 缓存命中 && !refresh → 任务直接 done 返回现有 epubUrl
...
class JobManager {
  ...
  async create({ source, sourceId, refresh }) {
    ...
    // 缓存命中 && !refresh → 立即 done（契约 2.4）
    if (hasCache && !refresh) {
      const job = this._newJob({ source, sourceId, bookId });
      job.status = "done";
      job.epubUrl = epubUrl;
      // 缓存命中：从 EPUB 元数据补全真实章数（契约 2.4：done 时 epubUrl 非空即可）
      job.progress = await readEpubProgress(epubFile);
    ...
    // 异步执行，不阻塞响应
    this._run(job, adapter, { refresh, epubFile, epubUrl }).catch((err) => {
      // 兜底：确保 job 落到 failed
      if (job.status !== "done") {
        job.status = "failed";
  ...
  async _run(job, adapter, { refresh, epubFile, epubUrl }) {
    job.status = "running";
    job.updatedAt = Date.now();
