...
class JobManager {
   ...
   * @param {number} [from] 章节范围起点（0 起始，含），仅下载该区间
   * @param {number} [to]   章节范围终点（0 起始，含）
   * @returns {{jobId:string, job:object}}
   */
  async create({ source, sourceId, refresh, from, to }) {
    ...
    const job = this._newJob({ source, sourceId, bookId, from, to });
    this.jobs.set(job.jobId, job);
    this.activeByBook.set(bookId, job.jobId);

    // 数据丢失防护：部分下载（带范围）且全书已缓存时，拒绝切片，避免覆盖已有全书。
    // 调用方应改用整本/增量更新路径。
    if (range && hasCache) {
      job.status = "failed";
      job.error = { code: "BAD_REQUEST", message: "该书已缓存全书，请使用整本更新或增量更新", detail: {} };
    ...
    // 异步执行，不阻塞响应
    this._run(job, adapter, { refresh, epubFile, epubUrl, hasCache, range }).catch((err) => {
      // 兜底：确保 job 落到 failed
      if (job.status !== "done") {
        job.status = "failed";
  ...
  _newJob({ source, sourceId, bookId, from, to }) {
  ...
  async _run(job, adapter, { refresh, epubFile, epubUrl, hasCache, range }) {
    job.status = "running";
    job.updatedAt = Date.now();
    ...
    try {
      ...
      // 部分下载：仅取 [from..to] 区间（clamp 到合法范围），EPUB 只含该切片
      let slice = catalog;
      if (range) {
        slice = catalog.slice(range.from, range.to + 1);
        if (slice.length === 0) throw new Error("章节范围越界");
      }
      const total = slice.length;
      job.progress.total = total;

      // 增量更新：无范围 && 非强制刷新 && 已有缓存时，比对章节数——相同命中缓存（不重抓）。
      // 部分下载带范围，始终按该区间重抓（不与整本缓存比对）。
      if (!range && !refresh && hasCache) {
...
/**
 * 归一化部分下载的章节范围。
 * from/to 都必须是 ≥0 的整数；缺任一视为无范围（整本下载）。
 * 返回 {from,to} | null。
 */
function normalizeRange(from, to) {
  ...
  const f = Number(from);
  const t = Number(to);
  if (!Number.isInteger(f) || !Number.isInteger(t) || f < 0 || t < 0 || f > t) return null;
  return { from: f, to: t };
}

module.exports = { JobManager, rememberSearchMeta, toErrorEnvelope, normalizeInfo, normalizeRange };