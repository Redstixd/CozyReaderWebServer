...
/**
 * 执行规则里 `@js:` 后缀的 JS 后处理程序（so-novel StepEngine 语义）。
 * 程序形如 `var qsbs={...};r=r.replace(...)`：在沙箱里预置 `r` 为输入串，
 * 运行后读回 `r`。用于 ranwen8 等把正文 base64 混淆的站点。
 */
function runJsDsl(code, input) {
  const sandbox = { r: input, console: { log() {}, warn() {} } };
  try {
    vm.createContext(sandbox);
    vm.runInContext(code, sandbox, { timeout: 1000 });
  } catch {
    return input; // 执行失败则原样返回，避免白屏
  }
  return typeof sandbox.r === "string" ? sandbox.r : input;
}

/**
 * 取 content 选择器的 innerHTML，并处理 so-novel 的 DSL 后缀：
 *  - `@java:base64.decode()`：提取 document.writeln(qsbs.bb('...')) 里的 base64 串拼接解码
 *  - `@js:...`：执行内嵌 JS 程序对 innerHTML 做后处理（如 ranwen8 的正文解码）
 */
function extractHtmlContent($doc, contentSel) {
  ...
  if (!el || el.length === 0) return "";
  let html = el.html() || "";
  const jsDsl = String(q).match(/@js:([\s\S]+)$/);
  if (jsDsl && jsDsl[1]) {
    html = runJsDsl(jsDsl[1], html);
    return html;
  }
